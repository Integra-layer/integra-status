...
import type { Category, Environment, Endpoint, AppGroup, Owner, CommonIssue } from "./types";

// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------

export const CATEGORIES: Category[] = ["blockchain", "validators", "apis", "frontends", "external"];
...
const evmRpcIssues: CommonIssue[] = [
  { cause: "intgd service stopped or crashed", fix: "SSH in and restart: sudo systemctl restart intgd" },
  { cause: "Disk full on EC2 instance", fix: "Check disk usage: df -h, clean up old logs or prune chain data" },
  { cause: "Node fell behind sync", fix: "Check sync status: curl localhost:8545 -X POST -H 'Content-Type: application/json' -d '{\"jsonrpc\":\"2.0\",\"method\":\"eth_syncing\",\"params\":[],\"id\":1}'" },
];

const evmRpcCaddyIssues: CommonIssue[] = [
  { cause: "intgd service stopped or crashed", fix: "SSH in and restart: sudo systemctl restart intgd" },
  { cause: "Caddy reverse proxy misconfigured or down", fix: "Check Caddy status: sudo systemctl status caddy; verify /rpc route uses handle_path to strip prefix" },
  { cause: "TLS certificate expired or renewal failed", fix: "Check Caddy logs: journalctl -u caddy -n 50; Let's Encrypt renewal should be automatic" },
];

const cosmosRpcIssues: CommonIssue[] = [
  { cause: "CometBFT process crashed or halted", fix: "SSH in and restart: sudo systemctl restart intgd; check logs: journalctl -u intgd -n 100" },
  { cause: "Node stuck at a block height (consensus stall)", fix: "Check latest block: curl localhost:26657/status | jq .result.sync_info; compare with peers" },
  { cause: "P2P port 26656 blocked by firewall", fix: "Check security group rules allow inbound TCP 26656; verify with: ss -tlnp | grep 26656" },
];

const cosmosRpcCaddyIssues: CommonIssue[] = [
  { cause: "CometBFT process crashed or halted", fix: "SSH in and restart: sudo systemctl restart intgd" },
  { cause: "Caddy reverse proxy not routing /cometbft correctly", fix: "Check Caddyfile route for /cometbft; ensure handle_path strips prefix before proxying to port 26657" },
  { cause: "TLS certificate expired or renewal failed", fix: "Check Caddy logs: journalctl -u caddy -n 50; Let's Encrypt auto-renewal should handle this" },
];

const cosmosRestIssues: CommonIssue[] = [
  { cause: "REST/LCD server not enabled or crashed", fix: "Verify api.enable = true in app.toml; restart: sudo systemctl restart intgd" },
  { cause: "Node is syncing and REST queries return stale data", fix: "Check sync status: curl localhost:26657/status | jq .result.sync_info.catching_up" },
  { cause: "Port 1317 blocked or not exposed", fix: "Check firewall rules and Caddy config for REST proxy route" },
];

const cosmosRestCaddyIssues: CommonIssue[] = [
  { cause: "REST/LCD server not enabled or crashed", fix: "Verify api.enable = true in app.toml; restart: sudo systemctl restart intgd" },
  { cause: "Caddy reverse proxy not routing /rest correctly", fix: "Check Caddyfile route for /rest; ensure handle_path strips prefix before proxying to port 1317" },
  { cause: "TLS certificate expired", fix: "Check Caddy logs: journalctl -u caddy -n 50" },
];

const validatorPeerIssues: CommonIssue[] = [
  { cause: "Validator not in peer list — node may be offline", fix: "Verify the validator is running and has peers: curl localhost:26657/net_info | jq '.result.peers | length'" },
  { cause: "Firewall blocking P2P port 26656", fix: "Check security group rules for port 26656 inbound; verify with: ss -tlnp | grep 26656" },
  { cause: "Validator jailed due to downtime", fix: "Check jail status: intgd query staking validator <valoper> --output json | jq .jailed; unjail if needed" },
];

const vercelFrontendIssues: CommonIssue[] = [
  { cause: "Vercel deployment failed", fix: "Check Vercel dashboard for build errors; redeploy with: vercel --prod" },
  { cause: "DNS resolution failure", fix: "Verify DNS records in Cloudflare/Route53 point to Vercel" },
  { cause: "Vercel platform outage", fix: "Check https://www.vercelstatus.com for platform-wide incidents" },
];

const deepHealthApiIssues: CommonIssue[] = [
  { cause: "Application crashed or OOM killed", fix: "Check application logs; restart the service; verify memory limits" },
  { cause: "Database connection pool exhausted", fix: "Restart the service to reset connections; check DB connection count and limits" },
  { cause: "Health endpoint returning unhealthy status", fix: "Check /health response for specific component failures (DB, cache, queue)" },
];

const httpReachableApiIssues: CommonIssue[] = [
  { cause: "Service unreachable or DNS failure", fix: "Verify DNS resolves correctly; check if service is running on the host" },
  { cause: "TLS/SSL certificate expired", fix: "Check certificate expiry; renew or redeploy" },
];

const externalServiceIssues: CommonIssue[] = [
  { cause: "Third-party service outage", fix: "Check their status page; no action needed, wait for recovery" },
  { cause: "API rate limit exceeded", fix: "Check rate limit headers in response; reduce polling frequency or upgrade plan" },
];
...
export const APP_GROUPS: AppGroup[] = [
  ...
  { id: "explorer", name: "Explorer", icon: "\uD83D\uDD0D", description: "Block explorer for transactions, blocks, and validators", endpoints: ["explorer-mainnet", "explorer-testnet", "explorer-v2", "explorer-graphql", "blockscout-mainnet", "blockscout-testnet", "mainnet-cosmos-rpc", "mainnet-cosmos-rest", "testnet-cosmos-rpc", "testnet-cosmos-rest"] },
  ...
  { id: "blockchain", name: "Blockchain Nodes", icon: "\u26D3\uFE0F", description: "EVM and Cosmos RPC/REST nodes for mainnet and testnet", endpoints: ["mainnet-evm-rpc", "mainnet-evm-rpc-adam", "mainnet-cosmos-rpc-adam", "mainnet-cosmos-rest-adam", "testnet-evm-rpc", "testnet-evm-rpc-ormos", "testnet-cosmos-rpc-ormos", "testnet-cosmos-rest-ormos"] },
  ...
  { id: "sites", name: "Websites & Docs", icon: "\uD83C\uDF10", description: "Marketing website, documentation, whitepaper, and staking", endpoints: ["main-website", "docs-site", "whitepaper", "staking", "datastore", "integra-connect", "status-page"] },
  { id: "standalone", name: "Other APIs", icon: "\u2699\uFE0F", description: "Supply data, file uploads, and utility endpoints", endpoints: ["supply-api", "presign-api", "upload-tracker-api", "testnet-faucet-api"] },
...
export const ENDPOINTS: Endpoint[] = [
  // -- Blockchain (Mainnet -- integra-1, EVM chain 26217) --------------------
  {
    ...
    category: "blockchain",
    environment: "prod",
    url: "https://evm.integralayer.com/api/eth-rpc",
    checkType: "evm-rpc",
    timeout: 10000,
    enabled: true,
    expectedChainId: "0x6669",
    dependsOn: [],
    impacts: ["dashboard-api-prod"],
    impactDescription: "Dashboard API loses EVM data — balances and transactions unavailable",
    description: "EVM JSON-RPC for mainnet (chain 26217) — wallet balances, transactions, smart contracts",
    richDescription: "The primary Ethereum-compatible JSON-RPC node for Integra mainnet (chain ID 26217), built on the Cosmos SDK EVM module. It serves all on-chain EVM data including wallet balances, token transfers, smart contract calls, and transaction receipts. The Dashboard API depends on this directly for portfolio valuations, staking position reads, and transaction history. If this endpoint goes down, the entire EVM data layer in the production dashboard becomes unavailable — users see stale balances and cannot submit transactions.",
    owner: OWNERS.adam,
    links: { endpoint: "https://evm.integralayer.com/api/eth-rpc", docs: "https://docs.integralayer.com/nodes" },
  ...
  },
  {
    ...
    richDescription: "WebSocket endpoint for real-time EVM event subscriptions on mainnet, enabling live monitoring of token transfers, contract events, and new blocks without polling. Currently disabled but planned for real-time dashboard features like live transaction feeds and instant balance updates. When activated, it will reduce latency for event-driven workflows from 30s polling intervals to sub-second push notifications.",
    owner: OWNERS.adam,
    links: { endpoint: "wss://ws.integralayer.com", docs: "https://docs.integralayer.com/nodes" },
    commonIssues: [
      { cause: "WebSocket server not started or crashed", fix: "SSH in and check if the WS listener is active on the expected port; restart intgd" },
      { cause: "Firewall blocking WebSocket port", fix: "Check security group rules for the WebSocket port; ensure wss:// upgrade is allowed through the proxy" },
    ],
    tags: ["EVM", "WebSocket"],
  },
  {
  ...
  },
  {
    ...
    description: "Cosmos REST/LCD for mainnet — governance, staking queries, account info",
    richDescription: "The Cosmos REST (LCD) endpoint for mainnet, serving governance proposals, staking delegation data, account balances, validator sets, and distribution rewards via standard HTTP REST calls. The explorer depends on this heavily for validator detail pages, governance proposal displays, and account lookup features. External integrations (wallets, aggregators) also use this endpoint for IRL token balance queries. If down, governance and staking data disappears from all consumer applications.",
    owner: OWNERS.adam,
    links: { endpoint: "https://api.integralayer.com", docs: "https://docs.integralayer.com/nodes" },
  ...
  {
    id: "mainnet-evm-rpc-adam",
    name: "Mainnet EVM RPC (Adam)",
    category: "blockchain",
    environment: "prod",
    ...
    dependsOn: [],
    impacts: ["explorer-v2"],
    impactDescription: "Explorer v2 loses EVM data if primary also down",
    description: "Adam's EVM RPC gateway — Caddy reverse proxy to local EVM node",
    richDescription: "Secondary EVM JSON-RPC endpoint for Integra mainnet, served via Caddy reverse proxy on Adam's AWS EC2 validator node (3.92.110.107). Provides geographic redundancy from the primary evm.integralayer.com endpoint. Also serves the Explorer v2 block indexer (Callisto) with EVM data for token transfers, balances, and event logs. Caddy handles TLS termination and path-based routing (/rpc strips prefix before proxying to port 8545).",
    owner: OWNERS.adam,
    links: { endpoint: "https://adamboudj.integralayer.com/rpc" },
  ...
  },
  {
    ...
    dependsOn: [],
    impacts: [],
    description: "Adam's REST/LCD gateway — Cosmos REST via Caddy reverse proxy",
    richDescription: "Secondary Cosmos REST/LCD endpoint for mainnet, served via Caddy on Adam's validator node. Provides governance, staking, and account queries as a fallback to the primary api.integralayer.com endpoint. Used by the Explorer v2 staking indexer for validator set queries every 100 blocks.",
    owner: OWNERS.adam,
  ...
  },
  {
    ...
    description: "Cosmos REST/LCD for testnet — development queries and testing",
    richDescription: "Cosmos REST/LCD for the Ormos testnet, serving governance, staking, and account queries over HTTP for development. The testnet explorer depends on this for displaying validator and governance data during QA. Also used by the backend team for integration testing Dashboard API features against realistic chain data before promoting to production.",
    owner: OWNERS.adam,
    links: { endpoint: "https://testnet-api.integralayer.com", docs: "https://docs.integralayer.com/nodes" },
    commonIssues: cosmosRestIssues,
    tags: ["Cosmos SDK"],
  },
  ...
  },
  {
    ...
    dependsOn: [],
    impacts: [],
    description: "Ormos testnet Cosmos RPC gateway — CometBFT via Caddy reverse proxy",
    richDescription: "Secondary CometBFT RPC endpoint for the Ormos testnet, proxied through Caddy on Adam's testnet validator node. Provides an alternative RPC path to the primary testnet-rpc.integralayer.com endpoint. Used by the IntegraWatch Telegram bot for /status validator monitoring.",
    owner: OWNERS.adam,
  ...
  },
  {
    ...
    dependsOn: [],
    impacts: [],
    description: "Ormos testnet REST/LCD gateway — Cosmos REST via Caddy reverse proxy",
    richDescription: "Secondary Cosmos REST/LCD endpoint for the Ormos testnet, served via Caddy on Adam's testnet validator node. Provides governance, staking, and account queries as a fallback to the primary testnet-api.integralayer.com endpoint.",
    owner: OWNERS.adam,
  ...
  },
  {
    ...
    richDescription: "Testnet faucet API running on Adam's Ormos validator node (13.218.88.209), proxied via Caddy at /api/faucet. Dispenses test IRL tokens to developer wallet addresses for testing smart contracts, transaction flows, and staking features on the Ormos testnet. Includes a cooldown timer to prevent abuse. Depends on the testnet Cosmos RPC for broadcasting faucet transactions.",
    owner: OWNERS.adam,
    links: { endpoint: "https://ormos.integralayer.com/api/faucet", docs: "https://docs.integralayer.com", repo: "https://github.com/Integra-layer/docs" },
    commonIssues: [
      { cause: "Faucet wallet out of test IRL tokens", fix: "Top up the faucet wallet from the testnet genesis account or mint new tokens" },
      { cause: "Caddy not routing /api/faucet correctly", fix: "Check Caddyfile route; ensure handle_path strips /api/faucet prefix before proxying" },
      { cause: "Cosmos RPC dependency down", fix: "Faucet cannot broadcast transactions without RPC; fix testnet-cosmos-rpc-ormos first" },
    ],
    tags: ["API", "Caddy", "AWS"],
  },
  ...
  },
  {
    ...
    richDescription: "Adam's personal validator node running on AWS EC2 in Singapore (ap-southeast-1), providing Asia-Pacific geographic diversity to the validator set. This is the only validator outside the DigitalOcean infrastructure, reducing single-provider risk. Bonded with 100 IRL tokens staked, actively signing blocks and earning staking rewards. Managed directly by Adam with separate SSH keys and monitoring — contact adam@integralayer.com for any issues with this specific node.",
    owner: OWNERS.adam,
    links: { endpoint: "http://adamboudj.integralayer.com:26657" },
    commonIssues: [
      { cause: "Validator not in peer list — node may be offline", fix: "SSH into AWS EC2 (3.92.110.107): ssh -i ~/.ssh/integra-validator-key.pem ubuntu@3.92.110.107; check: sudo systemctl status intgd" },
      { cause: "AWS EC2 instance stopped or terminated", fix: "Check AWS console for instance i-* in ap-southeast-1; restart if stopped" },
      { cause: "Validator jailed due to downtime", fix: "Check jail status: intgd query staking validator integravaloper124gllptlcu2ew5guxnyvcc483jwkwj8mejng0m; unjail with: intgd tx slashing unjail" },
    ],
    tags: ["CometBFT", "AWS"],
  },
  ...
  // -- Backend APIs ----------------------------------------------------------
  {
    ...
    richDescription: "The authentication gateway for Integra, handling user signup and login via Web3Auth wallet pregeneration and Google OAuth social login. Built with NestJS and PostgreSQL, it creates non-custodial Integra wallets automatically when users sign up — no seed phrase management required. Depends on both Web3Auth (for wallet creation) and Google OAuth (for social login). Currently disabled pending the next onboarding flow release, but when active, it is the single entry point for all new user registrations.",
    owner: OWNERS.nawar,
    links: { endpoint: "https://passport-apis.integralayer.com/health/database", docs: "https://docs.integralayer.com" },
    commonIssues: [
      { cause: "NestJS application crashed or OOM killed", fix: "Check application logs; restart the service; verify memory limits on the host" },
      { cause: "PostgreSQL database connection failed", fix: "Verify database is running and accessible; check connection string in environment variables" },
      { cause: "Web3Auth or Google OAuth dependency down", fix: "Check web3auth and google-oauth endpoint status; Passport cannot create wallets or authenticate without them" },
    ],
  ...
  },
  {
    ...
    description: "Dashboard backend — auth, assets, XP, staking, bulk imports (75+ endpoints)",
    richDescription: "The primary backend powering the Integra Dashboard with 75+ REST API endpoints, built with NestJS, PostgreSQL, and JWT authentication. Handles portfolio management, staking position tracking, XP/points integration, asset management, bulk CSV imports, and notification preferences. The production Dashboard frontend is 100% dependent on this — if it goes down, users see a blank dashboard. Depends on the Absinthe XP API for points and leaderboard data. Deep health checks verify database connectivity and cache availability.",
    owner: OWNERS.nawar,
    links: { endpoint: "https://dashboard-apis.integralayer.com", docs: "https://docs.integralayer.com", repo: "https://github.com/Integra-layer/dashboard" },
    commonIssues: [
      { cause: "NestJS application crashed or OOM killed", fix: "Check application logs on the server; restart the service" },
      { cause: "PostgreSQL database connection pool exhausted", fix: "Restart the service to reset DB connections; check max_connections and pool size in config" },
      { cause: "Absinthe XP API dependency down", fix: "XP features will fail but core dashboard should still work; check absinthe-api status" },
    ],
  ...
  },
  {
    ...
    description: "City of Integra game backend — city builder logic, game state",
    richDescription: "The game backend for City of Integra, a gamified city builder experience hosted on AWS App Runner in ap-south-1 (Mumbai). Manages all game state including building placement, resource generation, city progression, upgrade paths, and player inventories. The City Builder frontend is 100% dependent on this service — without it, the game is completely unplayable. Deep health checks verify database connectivity and game state consistency. Contact Kalki for game logic issues or Nawar for infrastructure/deployment concerns.",
    owner: OWNERS.kalki,
    links: { endpoint: "https://ng6mpgxjz7.ap-south-1.awsapprunner.com", repo: "https://github.com/Integra-layer/city" },
    commonIssues: [
      { cause: "AWS App Runner service crashed or scaling issue", fix: "Check AWS App Runner console in ap-south-1; force new deployment if stuck" },
      { cause: "Database connection pool exhausted", fix: "Restart the App Runner service; check DB connection limits and pool configuration" },
      { cause: "Game state corruption or migration failure", fix: "Check application logs for schema errors; contact Kalki for game logic issues" },
    ],
    tags: ["AppRunner"],
  },
  {
    ...
    richDescription: "Development instance of the City of Integra game backend on AWS App Runner (Mumbai), used for testing new game mechanics, balance tuning, feature prototyping, and QA before production release. Runs against a separate dev database with test game state. If down, Kalki and the game team cannot test new features or validate bug fixes. Also used for load testing game logic before mainnet events.",
    owner: OWNERS.kalki,
    links: { endpoint: "https://ygmwadph3x.ap-south-1.awsapprunner.com", repo: "https://github.com/Integra-layer/city" },
    commonIssues: [
      { cause: "AWS App Runner dev service crashed", fix: "Check AWS App Runner console in ap-south-1; force new deployment" },
      { cause: "Dev database out of sync with schema", fix: "Run pending migrations; check for failed migration logs in App Runner" },
      { cause: "App Runner cold start timeout", fix: "Dev instances may scale to zero; retry request — App Runner will auto-provision" },
    ],
    tags: ["AppRunner"],
  },
  {
    ...
    richDescription: "A lightweight API endpoint serving the current circulating supply of IRL tokens, consumed by cryptocurrency exchanges (for market cap calculations), data aggregators like CoinGecko and CoinMarketCap, and portfolio trackers. Returns a single number but has outsized importance — incorrect or unavailable supply data causes wrong market cap figures across all listing platforms. Hosted on Polytrade infrastructure (legacy domain). Contact Parth for supply calculation logic updates.",
    owner: OWNERS.parth,
    links: { endpoint: "https://supply.polytrade.finance" },
    commonIssues: [
      { cause: "Polytrade infrastructure hosting issue", fix: "Contact Parth — this is hosted on legacy Polytrade infrastructure" },
      { cause: "Supply calculation returning incorrect value", fix: "Verify the supply calculation logic; check if chain data source is accessible" },
    ],
    tags: ["API"],
  },
  {
    ...
    richDescription: "Real-time token pricing API that aggregates IRL and related token prices from multiple sources, serving them to the Dashboard for portfolio valuations, staking reward estimates, and transaction value displays. If this endpoint goes down or returns stale data, users see incorrect portfolio totals and missing price charts in the production Dashboard. Hosted on Polytrade infrastructure. No external dependencies but impacts user confidence when prices appear stale or zero.",
    owner: OWNERS.parth,
    links: { endpoint: "https://price.api.polytrade.app" },
    commonIssues: [
      { cause: "Price aggregation source unreachable", fix: "Check upstream price feeds (CoinGecko, DEX); service may return stale cached prices" },
      { cause: "Polytrade infrastructure hosting issue", fix: "Contact Parth — this is hosted on legacy Polytrade infrastructure" },
    ],
    tags: ["API"],
  },
  ...
  },
  {
    ...
    timeout: 10000,
    enabled: true,
    dependsOn: ["mainnet-cosmos-rpc-adam", "mainnet-evm-rpc-adam"],
    impacts: [],
    description: "Explorer v2 — Big Dipper fork with Cosmos + EVM indexing",
    richDescription: "Full-rewrite block explorer (Big Dipper 2.0 fork) deployed on Adam's mainnet validator node (3.92.110.107) via Caddy reverse proxy. Built with Next.js 16 + React 19 + Tailwind v4 + Apollo Client 3. Backed by PostgreSQL 15 (54 tables), Hasura v2.29.0 GraphQL engine, and the Callisto Go indexer for both Cosmos and EVM data. Features gas analytics, address labels, contract verification, and validator staking views. Depends on Adam's CometBFT RPC and EVM RPC endpoints for live indexing.",
    owner: OWNERS.adam,
    links: { endpoint: "https://scan.integralayer.com", repo: "https://github.com/Integra-layer/bid-dipper" },
    commonIssues: [
      { cause: "Explorer web service (explorer-web.service) crashed", fix: "SSH in and restart: sudo systemctl restart explorer-web; check logs: journalctl -u explorer-web -n 50" },
      { cause: "Callisto indexer stopped or fell behind", fix: "Check Callisto service: sudo systemctl status callisto; restart if needed; check indexing lag in PostgreSQL" },
      { cause: "Hasura GraphQL engine down", fix: "Check Docker containers: docker ps | grep hasura; restart: docker compose -f /path/to/docker-compose.yml restart hasura" },
    ],
  ...
  },
  {
    ...
    impacts: [],
    description: "Hasura GraphQL API for Explorer v2 — blocks, transactions, validators",
    richDescription: "Hasura v2.29.0 GraphQL engine serving the Explorer v2 data layer. Tracks 51 PostgreSQL tables and 3 analytics functions (recent_gas_stats, gas_price_tiers, gas_price_by_day). Provides real-time queries for blocks, transactions, validators, staking data, EVM tokens, and address labels. Used by the Explorer v2 frontend via Apollo Client. Health depends on both PostgreSQL and the Callisto indexer keeping data fresh.",
    owner: OWNERS.adam,
    links: { endpoint: "https://scan.integralayer.com/v1/graphql", repo: "https://github.com/Integra-layer/bid-dipper" },
    commonIssues: [
      { cause: "Hasura container crashed or out of memory", fix: "Check: docker ps | grep hasura; restart: docker compose restart hasura" },
      { cause: "PostgreSQL database connection failed", fix: "Check PostgreSQL container: docker ps | grep postgres; verify connection in Hasura environment variables" },
      { cause: "Hasura metadata inconsistency", fix: "Re-apply metadata: run /tmp/apply_hasura_metadata.py on the server; check for tracked table/function mismatches" },
    ],
  ...
  },
  {
    ...
    timeout: 10000,
    enabled: true,
    dependsOn: ["mainnet-evm-rpc-adam"],
    impacts: [],
    description: "Blockscout EVM explorer for mainnet — EVM transactions, tokens, contracts",
    richDescription: "Blockscout EVM block explorer for Integra mainnet (chain ID 26217), deployed via Docker Compose on Adam's validator EC2 (3.92.110.107). Backend indexes EVM blocks, transactions, token transfers, and contract interactions via JSON-RPC. Frontend provides transaction search, address pages, token lists, gas tracker, and contract verification. Caddy reverse proxies blockscout.integralayer.com to the frontend (port 3002) and API (port 4000). Depends on the local EVM RPC node for indexing.",
    owner: OWNERS.adam,
    links: { endpoint: "https://blockscout.integralayer.com", repo: "https://github.com/Integra-layer/bid-dipper" },
    commonIssues: [
      { cause: "Blockscout Docker containers stopped", fix: "SSH in; check: docker ps | grep blockscout; restart: docker compose up -d blockscout" },
      { cause: "Blockscout indexer fell behind or stuck", fix: "Check indexer logs: docker logs blockscout-backend --tail 100; may need to restart backend container" },
      { cause: "Caddy not routing blockscout.integralayer.com correctly", fix: "Check Caddyfile for blockscout reverse proxy config; verify ports 3002 (frontend) and 4000 (API)" },
    ],
  ...
  },
  {
    ...
    impacts: [],
    description: "Blockscout EVM explorer for testnet Ormos — development EVM testing",
    ...
    owner: OWNERS.adam,
    links: { endpoint: "https://testnet.blockscout.integralayer.com", repo: "https://github.com/Integra-layer/bid-dipper" },
    commonIssues: [
      { cause: "Blockscout Docker containers stopped on testnet EC2", fix: "SSH into 13.218.88.209; check: docker ps | grep blockscout; restart: docker compose up -d" },
      { cause: "Testnet Blockscout backend on port 4001 (not 4000)", fix: "Port 4000 is used by faucet on testnet; verify backend is on 4001 in docker-compose.yml" },
      { cause: "Caddy not routing testnet.blockscout.integralayer.com", fix: "Check Caddyfile on testnet server for blockscout reverse proxy routes" },
    ],
    tags: ["Blockscout", "Docker", "AWS"],
  },
  {
    ...
    dependsOn: ["vercel"],
    impacts: [],
    description: "Infrastructure status dashboard — real-time health monitoring",
    richDescription: "The Integra infrastructure status page deployed on Vercel at status.integralayer.com, monitoring 40+ endpoints across blockchain nodes, validators, APIs, frontends, and external services. Self-monitoring ensures the status page itself is accessible when users need to check system health during incidents.",
    owner: OWNERS.adam,
    links: { endpoint: "https://status.integralayer.com", repo: "https://github.com/Integra-layer/integra-status" },
    commonIssues: vercelFrontendIssues,
    tags: ["Vercel"],
  },
  {
  ...
  },
  {
    ...
    richDescription: "Development instance of the Dashboard frontend on Vercel, connected to the dev Dashboard API and testnet blockchain endpoints. Used by Tara and the frontend team for building and testing new UI features, validating design changes, and running E2E tests before production deployment. Also serves as the staging environment for stakeholder previews. If down, frontend development is blocked and the team cannot demo upcoming features.",
    owner: OWNERS.tara,
    links: { endpoint: "https://dev-dashboard.integralayer.com", docs: "https://docs.integralayer.com", repo: "https://github.com/Integra-layer/dashboard" },
    commonIssues: vercelFrontendIssues,
    tags: ["React", "Vercel"],
  },
  {
    ...
    owner: OWNERS.kalki,
    links: { endpoint: "https://city.integralayer.com", repo: "https://github.com/Integra-layer/city" },
    commonIssues: vercelFrontendIssues,
    tags: ["React", "Vercel"],
  },
  {
    ...
    richDescription: "Development instance of the City of Integra frontend on Vercel, used by Kalki for testing new game UI features, building animations, and UX improvements before production. Connected to the dev City API with test game state. Also used for playtesting new mechanics and balance changes with the team before releasing to users.",
    owner: OWNERS.kalki,
    links: { endpoint: "https://dev-city.integralayer.com", repo: "https://github.com/Integra-layer/city" },
    commonIssues: vercelFrontendIssues,
    tags: ["React", "Vercel"],
  },
  {
  ...
  },
  {
    ...
    richDescription: "The staking interface built with React on Vercel (Polytrade legacy domain), where users delegate IRL tokens to validators, manage their delegation positions, redelegate between validators, and claim accumulated staking rewards. This is critical for network security — it is the primary way token holders participate in consensus by staking. If down, users cannot delegate or claim rewards, potentially impacting validator economics and network security participation rates. Contact Tara for frontend issues.",
    owner: OWNERS.tara,
    links: { endpoint: "https://staking.polytrade.finance" },
    commonIssues: [
      { cause: "Vercel deployment failed", fix: "Check Vercel dashboard for build errors; redeploy with: vercel --prod" },
      { cause: "Legacy Polytrade domain DNS issue", fix: "Verify DNS records for staking.polytrade.finance point to Vercel" },
      { cause: "Vercel platform outage", fix: "Check https://www.vercelstatus.com for platform-wide incidents" },
    ],
    tags: ["React", "Vercel"],
  },
  {
    ...
    richDescription: "Legacy landing page from the Polytrade Finance era, currently disabled in monitoring. Traffic is being migrated to the new integralayer.com domain. Will be fully deprecated once DNS redirects are confirmed and all inbound links are updated. Keeping the endpoint registered for tracking purposes until deprecation is complete.",
    owner: OWNERS.tara,
    links: { endpoint: "https://get.polytrade.finance" },
    commonIssues: [
      { cause: "Legacy page still receiving traffic after deprecation", fix: "Set up DNS redirect from get.polytrade.finance to integralayer.com" },
      { cause: "Vercel deployment issue", fix: "Check Vercel dashboard; this endpoint is disabled and may not need action" },
    ],
    tags: ["Vercel"],
  },
  {
    ...
    richDescription: "NFT metadata and media storage service hosted on Vercel, serving asset images, metadata JSON files, and collection information for NFT display across the Dashboard and marketplace features. Follows the ERC-721/1155 metadata standard so wallets and aggregators can resolve token URIs. If down, NFT thumbnails and metadata fail to load across the platform — users see broken images and missing asset names. Contact Tara for deployment or Parth for metadata schema changes.",
    owner: OWNERS.tara,
    links: { endpoint: "https://datastore.polytrade.app" },
    commonIssues: [
      { cause: "Vercel deployment failed or build error", fix: "Check Vercel dashboard for build errors; redeploy" },
      { cause: "NFT metadata format changed breaking consumers", fix: "Verify ERC-721/1155 metadata schema compliance; check token URI resolution" },
      { cause: "Legacy Polytrade domain DNS issue", fix: "Verify DNS records for datastore.polytrade.app" },
    ],
    tags: ["Vercel"],
  },
  ...
  // -- External Dependencies -------------------------------------------------
  {
    ...
    richDescription: "The primary GitHub organization for Integra Layer, hosting all active repositories including the explorer, dashboard, city builder, validator tooling, and infrastructure-as-code. Monitored via the GitHub API to detect outages that would block CI/CD pipelines, code deployments, PR reviews, and developer collaboration. A GitHub outage halts all development velocity across the team. Contact Adam for org-level access issues.",
    owner: OWNERS.adam,
    links: { endpoint: "https://api.github.com/orgs/Integra-layer", repo: "https://github.com/Integra-layer" },
    commonIssues: [
      { cause: "GitHub platform outage", fix: "Check https://www.githubstatus.com; no action needed, wait for recovery" },
      { cause: "GitHub API rate limit exceeded", fix: "Check rate limit headers: curl -I https://api.github.com/rate_limit; use authenticated requests for higher limits" },
    ],
    tags: ["GitHub"],
  },
  {
    ...
    richDescription: "Legacy GitHub organization from the Polytrade Finance era, still hosting shared libraries, older smart contracts, and utility packages that some services depend on. Monitored to ensure CI/CD pipelines that pull from these repos continue to function. Being gradually migrated to the Integra-layer org. Contact Adam for access or migration questions.",
    owner: OWNERS.adam,
    links: { endpoint: "https://api.github.com/orgs/polytrade-finance", repo: "https://github.com/polytrade-finance" },
    commonIssues: [
      { cause: "GitHub platform outage", fix: "Check https://www.githubstatus.com; no action needed, wait for recovery" },
      { cause: "Legacy repo access permissions changed", fix: "Verify org membership and repo access; contact Adam for org admin changes" },
    ],
    tags: ["GitHub"],
  },
  {
  ...
  },
  {
    ...
    description: "OpenAI — AI-powered title deed extraction and document processing",
    richDescription: "OpenAI API integration (GPT-4 Vision) used in the Dashboard for AI-powered title deed extraction and document processing. When users upload property documents, the system uses GPT-4V to perform OCR, extract structured data (owner name, property details, valuation), and auto-populate form fields. If OpenAI is down, document uploads still succeed but automatic data extraction fails — users must manually enter all property information. Non-blocking but degrades UX significantly. Contact Parth for API key or model configuration.",
    owner: OWNERS.parth,
    links: { endpoint: "https://api.openai.com/v1/models" },
    commonIssues: [
      { cause: "OpenAI platform outage or degraded performance", fix: "Check https://status.openai.com; document extraction will fail but uploads still work" },
      { cause: "OpenAI API key expired or rate limited", fix: "Check API key in environment variables; contact Parth for key rotation or plan upgrade" },
      { cause: "GPT-4V model not available or deprecated", fix: "Check available models at api.openai.com/v1/models; update model ID in config if needed" },
    ],
    tags: ["AI", "GPT"],
  },
  {
  ...
  },
  {
    ...
    richDescription: "Twitter/X API v2 integration for displaying the Integra social media feed on the Dashboard, showing recent announcements, community highlights, and ecosystem updates. While non-critical to core functionality, the social feed is prominently placed on the Dashboard home screen and its absence is visually noticeable to users. Frequent Twitter API rate limits or outages cause the feed section to show an empty state. Contact Tara for frontend display issues or Parth for API credential management.",
    owner: OWNERS.parth,
    links: { endpoint: "https://api.twitter.com/2" },
    commonIssues: [
      { cause: "Twitter/X API rate limit exceeded", fix: "Check rate limit headers; reduce polling frequency; Twitter free tier has very low limits" },
      { cause: "Twitter/X API key expired or app suspended", fix: "Check Twitter developer portal for app status; contact Parth for credential updates" },
      { cause: "Twitter/X platform outage", fix: "Check https://api.twitterstat.us; the dashboard feed will show empty state — no action needed" },
    ],
    tags: ["OAuth"],
  },
  {
    ...
    richDescription: "Google OAuth 2.0 / OpenID Connect endpoint used for social login authentication in the Passport API. When users click \"Sign in with Google,\" the authentication flow hits this endpoint for token exchange and identity verification. If Google OAuth is down (extremely rare), all Google-based sign-ups and logins fail — users must wait or use alternative login methods. The Passport API checks the OIDC discovery document on startup to cache signing keys. Contact Parth for OAuth client configuration or redirect URI changes.",
    owner: OWNERS.parth,
    links: { endpoint: "https://accounts.google.com/.well-known/openid-configuration" },
    commonIssues: [
      { cause: "Google OAuth platform outage (extremely rare)", fix: "Check https://www.google.com/appsstatus; wait for recovery — no action on our side" },
      { cause: "OAuth client ID or redirect URI misconfigured", fix: "Check Google Cloud Console for OAuth client settings; contact Parth for credential changes" },
    ],
    tags: ["OAuth", "OIDC"],
  },
  {
