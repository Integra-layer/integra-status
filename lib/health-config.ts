...
export const ENDPOINTS: Endpoint[] = [
  ...
  },
  {
    ...
    description: "Cosmos RPC for testnet — Hetzner Helsinki gateway",
    richDescription:
      "CometBFT RPC for the Integra testnet chain, served via Caddy path routing on the Helsinki gateway (46.225.231.81). Provides block data, transaction broadcasting, and consensus state for development and QA. Powers the testnet block explorer with live chain data.",
    owner: OWNERS.adam,
    links: {
      endpoint: "https://testnet.integralayer.com/rpc",
      docs: "https://docs.integralayer.com/nodes",
    },
    commonIssues: cosmosRpcCaddyIssues,
  ...
  },
  {
    ...
    category: "blockchain",
    environment: "dev",
    url: "https://testnet.integralayer.com/api",
    checkType: "cosmos-rest",
    timeout: 10000,
    enabled: true,
    dependsOn: ["portal-testnet"],
    ...
    description:
      "Cosmos REST/LCD for testnet — Hetzner Helsinki gateway",
    richDescription:
      "Cosmos REST/LCD for the Integra testnet, served via Caddy path routing on the Helsinki gateway (46.225.231.81). Serves governance, staking, and account queries over HTTP for development.",
    owner: OWNERS.adam,
    links: {
      endpoint: "https://testnet.integralayer.com/api",
      docs: "https://docs.integralayer.com/nodes",
    },
    commonIssues: cosmosRestCaddyIssues,
  ...
  },
  {
    ...
    impacts: [],
    description:
      "Blockscout EVM explorer for testnet — Hetzner Helsinki gateway",
    richDescription:
    ...
    commonIssues: [
      {
        cause: "Blockscout Docker containers stopped on Helsinki gateway",
        fix: "SSH into 46.225.231.81: ssh -i ~/.ssh/integra root@46.225.231.81; check: docker ps | grep blockscout; restart: docker compose up -d",
      },
      {
        cause: "Blockscout backend on port 4001",
        fix: "Verify backend is on 4001 in docker-compose.yml; check: curl localhost:4001/api/v2/stats",
      },
      {
        cause: "Caddy not routing testnet.blockscout.integralayer.com",
        fix: "Check Caddyfile on Helsinki (46.225.231.81) for blockscout reverse proxy routes",
      },
    ],
    tags: ["Blockscout", "Docker", "AWS"],
  },
